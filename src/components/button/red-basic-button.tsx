...
const RedBasicButton = ({ children }: { children: React.ReactNode }) => {
  ...
  const isProfileIncomplete =
    ...
    !userProfile.birth?.year ||
    !userProfile.birth?.month ||
    !userProfile.birth?.day;
  ...
  const handleUpdateProfile = async () => {
    ...
    const response = await axios.patch("/v1/users/me", {
      data: {
        ...
        birth: `${userProfile.birth.year}-${userProfile.birth.month}-${userProfile.birth.day}`,
        calendarType: userProfile.birth.calendarType,
      },
    });
